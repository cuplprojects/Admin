...
const ImportOmr = () => {
  ...
  const uploadFile = async (file, replace = false) => {
    try {
      ...
      const response = await axios.post(
        `${apiurl}/OMRData/upload-request?ProjectId=${ProjectId}`,
        { omrImagesName: fileNameWithoutExtension, filePath: base64File, replace: replace },
        {
          headers: {
            'Content-Type': 'application/json',
      ...
      await removeFromLocalForage(file); // Remove the file from localforage after successful upload
      return true;
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    const uploadPromises = files.map((file, index) => uploadFile(file));

    try {
      await Promise.all(uploadPromises);
      setCurrentFileIndex(files.length); // Set currentFileIndex to indicate all files are uploaded
      await removeCurrentFileIndex();
      setAlertMessage('');
      setAlertType('');
      setShowSkipBtn(false);
  ...
  const replaceFile = async () => {
    setShowReplaceBtn(false);
    setShowSkipBtn(false);
    await uploadFile(files[currentFileIndex], true);
    handleSubmit({ preventDefault: () => {} });
  };
  ...
  return (
    <div>
      ...
      {showSkipBtn && (
        <Button type="danger" onClick={skipFile}>
          Skip {files[currentFileIndex].name}
        </Button>
      )}
      {showReplaceBtn && (
        <Button type="primary" onClick={replaceFile}>
          Replace {files[currentFileIndex].name}
        </Button>
      )}
      {loading && (
        <Progress percent={progress} status="active" />
