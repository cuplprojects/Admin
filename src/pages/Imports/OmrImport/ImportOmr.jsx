...
const ImportOmr = () => {
        ...
        { omrImagesName: fileNameWithoutExtension, filePath: base64File, replace: replace },
        {
          ...
          onUploadProgress: (progressEvent) => {
            if (progressEvent.lengthComputable) {
              ...
              const totalLength = parseInt(progressEvent.target.getResponseHeader('content-length')) ||
                parseInt(progressEvent.target.getResponseHeader('x-decompressed-content-length')) || 10000; // Some default value
              progressPercent = Math.round((progressEvent.loaded * 100) / totalLength);
            }
  ...
  return (
    <div>
      ...
      {alertMessage && (
        <div className={`alert alert-${alertType} my-5`} role="alert">
          {alertMessage}
        </div>
      )}
